...
public class ImageEnhancerWithUndoAndRedo extends Component implements ActionListener {
    ...
    public ImageEnhancerWithUndoAndRedo() {
        ...
        try {
            ...
            biWorking = biTemp; // update biWorking with the original image
        } catch (IOException e) {
    ...
    public void actionPerformed(ActionEvent e) {
    	...
    	BufferedImage tempBufferedImage; // store temporary bufferedImage for undo/redo
    	if(e.getSource()==undoItem) {
    		tempBufferedImage = undoStack.pop(); // store the previous image
    		redoStack.push(biWorking); // push the current image to the redoStack
    		gWorking.drawImage(tempBufferedImage, 0, 0, null); // draw the previous image
    		biWorking = tempBufferedImage; // update the current image
    	}
    	else if(e.getSource()==redoItem) {
    		tempBufferedImage = redoStack.pop(); // store the redo image temporarily 
    		undoStack.push(biWorking); //push the current image to the undoStack
    		gWorking.drawImage(tempBufferedImage, 0, 0, null); // draw the redo image
    		biWorking = tempBufferedImage; // update the current image
    	} else { // if new operation is executed by the user...
    		redoStack.empty(); // empty the redoStack
    		undoStack.push(biWorking); // push the current image to the undoStack
    	}
        //  Also add code to enable and disable the Undo and Redo menu items, and to process
        //  these items when the user selects them.
    	if(redoStack.getSize() > 0) { // if redoStack is not empty...
    		redoItem.setEnabled(true); //redoItem is enabled
    	} else {
    		redoItem.setEnabled(false); // disable redoItem
    	}
    	
    	if(undoStack.getSize() > 0) { // if undoStack is not empty...
    		undoItem.setEnabled(true); // undoItem is enabled
    	} else {
    		undoItem.setEnabled(false); // undoItem is disabled
    	}
    	...
    	if (e.getSource()!=undoItem && e.getSource()!=redoItem) { // if user do new executions on the image
    		gWorking.drawImage(biFiltered, 0, 0, null); // Draw the pixels from biFiltered into biWorking.
    		biWorking = biFiltered; // update biWorking
    	}
